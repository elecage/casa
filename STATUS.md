...
2. **지표 구현 — 범위는 계산 가능한 것 전부(34종), 4종이 아니다.**
   목록을 48종으로 넓힌 목적이 "몇 개만 시험하고 적당한 지표가 없다고
   결론 내리는 것"을 막는 것이었다. 비용도 문제가 안 된다 — 대부분이 공용
   기반(결과 본문·정규화·내용 해시·본 것 집합) 위의 계수기라 기반이 서면
   각각 몇 줄이다. **아래 목록은 범위가 아니라 순서다.** 먼저 만들 것:
   - **에러 후 행동 변경률**(9번) — 외부 근거가 가장 센 지표(성공 92% 대
     실패 37%). 진전은 "에러가 새것인가"를 묻고 이 지표는 "에러를 보고
     행동을 바꿨는가"를 묻는다. 다른 질문이므로 대체되지 않는다.
   - **행동 패턴 순환**(3번) — 길이 3 이상 묶음의 반복. 구조 탐지라 별개.
   - **읽기 편중**(31번) — 상태 변경 없이 읽기만 이어지는 정도.
   - (헛된 확인 27번은 산출 0 + 증거 0으로 대부분 나오나, "결과를 다음
     행동에 반영했는가" 부분은 별도 판정이 필요하다.)
   그다음 **나머지 전부**. 그리고 **시작 전에 주 지표와 탐색 지표를 나눠
   결정 로그에 등록**한다 — 34종을 결과와 대조하면 몇 개는 우연히 맞으므로,
   전부 계산·전부 보고하고 다른 조건에서 확인한다(사후 조작 방지).
