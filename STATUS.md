...
**본 실험 재설계 방향 유저 결정 대기.** W9 분석 완료 — 원분석(§1~6),
재검토(§7~8), 측정 도구 교정(§9), 주장-실제 간극(§10). 유저와의 왕복으로
정리된 재설계 골격: 결과변수 3축 — (1) **실제 달성**(숨은 오라클 채점),
(2) **능력/효율**(완료까지 토큰·턴 — 같은 과제 성공끼리도 2~4배 차이,
§8), (3) **주장-증거 정합**(허위 완료·무보고 정지·mock 검출 — 실패
13건 중 12건이 완료 단언, §10). 시간 지표 금지(결정 로그). 파일럿 최강
신호는 숨은 오라클 과제의 수정→테스트 사이클(교정 후 AUROC 1.0, n 작음
— §9). 착수 대기 작업(W10 후보): RESEARCH_PLAN/PILOT_DESIGN 3축 개정,
저장 audit 재생성(셸 인지 파서로), 주장-정합 감사를 casa 코어 지표로
승격, in-the-wild(유저 실제 프로젝트 세션) arm 검토. 유저 승인 필요.
