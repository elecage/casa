...
**W8 (본 수집) — 유저 승인 대기.** G2 통과 (게이트 기록 참조). 구성:
과제 4종(A/B/C/D) × 15세션 × sonnet = 60세션, 예상 ~$25~30, 순차
~5~8시간 (백그라운드, 러너 크래시-재개 검증됨). 실행 전: `claude auth
status` 확인, `results/`를 .gitignore에 추가. 수집 중/직후 W6(casa
report 집계: 분산 통계, AUROC@k+Brier/ECE, 베이스라인, diff 통계) 착수.
보정 세션(scratchpad)은 본 데이터에 불포함.
