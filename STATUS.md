...
**→ W15: 능력 축 조기 탐지를 기존 127세션에 처음 계산함. 결과 = "예측해서
재시작"은 대체로 성립하지 않고, 성립하는 조건이 하나 있으나 근거가 세션
1개에 걸려 있다. 다음은 유저와 방향 결정.**

### 왜 이 계산을 했나 (방향 정정, 2026-08-19)

유저 지적: 지금까지의 결론이 유저의 원래 목표에서 벗어났다. 기록된 목표
(`memory/casa-research-focus.md`, 2026-07-22 명시 → 07-24 정교화)는
**"같은 프로젝트에서 새 세션마다 업무 처리 능력이 다르다 → 초반에 판별해
끊고 재시작하면 토큰·시간을 아낀다"**이고, 능력의 정의는 통과/실패가 아니라
**효율(같은 일을 몇 턴·몇 토큰)과 주장-실제 간극**이다. 그런데 실제로
계산된 조기 예측은 전부 통과/실패 표적이었다. `RESEARCH_PLAN.md`는 이미
2026-07-24에 예측 대상으로 "성공하지만 고비용"·"허위 완료 위험"을 추가해
두었는데(계획서는 옳았다), 2차 수집 이후 분석은 그걸 계산하지 않았다
(`signal_validation.py`는 세션 종료 후 지표만 사용, 초반 k스텝 로직 없음).
→ **"조기 탐지 불가"는 검정된 적 없는 주장이었다.** 신규 수집 없이 계산:
`pilot/analysis/ability_early.py` (+`tests/test_ability_early.py`).

재현: `.venv\Scripts\python.exe pilot/analysis/ability_early.py results/main2/*
--tasks-root pilot/tasks`

### 실측 결과 (조건별, 신규 수집 0)

능력 축에 변동은 확실히 있다 — 한 조건 안에서 최종 출력 토큰이
궤도(orbit) 27~60,527 / 원장 84~13,213 / 일정 2,858~10,669로 벌어진다.

**(1) 조기 신호의 판별력** (AUROC = 판별력, 0.5는 동전던지기):
고비용 꼬리를 예측하는 신호는 **"이미 태운 출력 토큰" 하나뿐**이다 —
스텝 2에서 0.50~0.69, 스텝 8에서 0.78~0.88. **행동 신호(탐색량, 읽은 파일,
커버리지, 반복, 테스트 실행, 자체검증, 규칙 위반)는 전부 0.5 근처 = 무신호.**
허위 완료 예측도 마찬가지로 약하며(일정 조건의 오류 누적 0.79가 최대,
단 30세션 중 26이 양성이라 불안정) 조건마다 다른 신호가 뜬다.

**(2) 재시작 규칙 = 대체로 손해.** 성공당 토큰 (귀무 = 신호 없이 전부 죽이기):

| 조건 | 아무것도 안 함 | 전부 죽이기 | 최적 탐지기 | 부작용 |
|---|---|---|---|---|
| 일정/haiku (성공 1/30) | 179,521 | 173,945 | **132,611 (+26%)** | 12죽임, 성공 0죽임 |
| 궤도/sonnet (성공 5/60) | 219,327 | 324,784 | 215,822 (+1.6%) | 7죽임, 성공 0죽임 |
| 원장/haiku (성공 26/30) | 8,235 | 8,688 | 7,827 (+5%) | 4죽임, **그중 3이 성공** |

즉 **성공률이 높은 조건에서는 죽이면 성공을 죽이고, 성공률이 바닥인 조건에서는
교체 세션도 실패하므로 아무 정책도 도움이 안 된다.** 유일한 +26%(일정)조차
이점의 전부가 "유일한 성공 세션 1개가 임계 밑에 있었다"에 걸려 있어 기술이
아니라 운일 수 있다.

**(3) 상한 회계 — 애초에 회수 가능한 돈이 적다.** 스텝 8 이후에 태워지는
토큰 비중(실패할 세션 = 회수 가능 최대 / 성공할 세션 = 멈추면 파괴):
원장 9% 대 61%(구조적으로 불가), 궤도 13% 대 10%(마진 없음),
일정 31% 대 1%(유일하게 유리), 표 분류 54% 대 29%(유망하나 n=7).
**소비가 앞쪽에 몰려 있어(궤도 1번 세션은 스텝 2까지 이미 78% 소비)
탐지가 정확해져도 회수할 잔액 자체가 없다.**

**분석 중 잡은 버그 3건**(전부 결론을 바꿨음, 테스트로 고정): 트랜스크립트가
한 메시지를 콘텐츠 블록마다 반복 기록해 단순 합산이 토큰을 3배로 부풀림
(43,131 대 CLI 자체 보고 15,906) / 재시작 시뮬레이션에서 스텝 k 전에 끝난
세션을 빼면 긴 세션만으로 정책이 평가돼 유리하게 왜곡 / 임계값을 2~3점에
맞추는 것 금지.

### 해석과 다음 선택지 (유저 결정 대기)

정직한 결론: **"실패를 예측해 재시작"이라는 형태로는 토큰 절약이 성립하지
않는다.** 근거는 세 겹 — 행동 신호 무신호, 회수 가능 잔액 부족, 재시작의
기대가치가 성공률 양극단에서 모두 무너짐. 다만 데이터가 지지하는 **다른
형태**가 하나 보인다: **결과 예측이 아니라 진행 중 병리 탐지.** 오늘 발견한
ml-shift 7번 세션(툴 호출 248회, 동일 호출 38회 반복, 86분 후 타임아웃,
규칙 위반 58건 — 나머지 6세션은 위반 0)처럼 퇴화한 세션은 결과와 무관하게
꼬리 소비를 만들고, 반복·오류 누적·토큰 급증으로 진행 중에 보인다. 이건
"이 세션이 실패할까"를 맞히지 않고도 낭비를 자른다.

선택지: (a) 병리 탐지로 표적 재정의(위 데이터가 지지, 기존 수집 재사용
가능), (b) 능력 축 표본을 늘려 재검정(표 분류가 회수 가능 54%로 유일하게
유망 — 단 힌트 문제 먼저 해결, 아래 참조), (c) 정직한 제약 결과로 집필.

### 힌트 제거판 프로브 결과 (`results/cal/tabular-b`, 2026-08-19)

**0/3 실패, 세 세션 모두 미공개 데이터 성능이 정확히 0.6159로 동일**, 각
73~82초·$0.17~0.20, 최종 출력 토큰 1,513~1,868(1.2배 편차). 힌트가 있던
`ml-shift`(보정 3 + 미기록 배치 7 = 10세션)는 성공 5, 249~455초,
$0.43~0.77이었다.

**해석:** 힌트를 빼면 세 세션 모두 **아무 조사도 하지 않고** 전체 피처를 그대로
써서 함정에 빠졌고 서로 완전히 동일했다. 즉 `ml-shift`에서 관측된 세션 간
변동은 코딩 실력 차이가 아니라 **"프롬프트에 있는 단서를 집어 행동으로
옮기는가"의 차이**였다. 이는 유저의 원래 현상 서술("같은 프롬프트인데 세션
마다 다르다")에 오히려 더 가까운 형태다. 단 n=3이므로 방향 단서.
**본 수집 조건 결정은 (a)/(b) 방향 결정과 함께 유저 확인 필요** — 선등록한
기준상 "성공률이 눈에 띄게 떨어지면 중립판 채택"이나, 중립판은 0/3으로
변동 자체가 사라져 능력 축 표본으로는 쓸모가 없다. 힌트 있는 조건이 오히려
"단서 포착 능력의 세션 간 변동"을 재는 조건일 수 있다(설계 의도와 다름 —
결정 로그 필요).

### 미기록 배치 발견 (`results/main2/ml-shift-sonnet`, 7세션)

2026-07-25 18:41~21:34 수집분이 STATUS에 없었다. 힌트 있는 조건, 상위 모델
7세션: 성공 3, 함정 실패 3(0.637/0.639/0.640), 기술적 실패 1(산출물 없음,
53초 조기 중단). 7번 세션이 위의 퇴화 사례. **기술적 실패를 성공률에서
분리하는 규칙이 아직 명문화되지 않았다**(현재는 배치마다 즉석 판단) — 결정
필요 항목.
