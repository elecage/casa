...
효율 축 표본으로 확정 전용. **결정 완료(A+B 병행) → 약모델 arm 검증 후 본 수집 설계(W13).** 진자
실측이 "orbit류 신규 과제 제작 가능" 전제를 뒤집어(§8), 유저가 A+B 병행
확정. 진행 순서:
1. **약모델 arm 검증** — haiku가 트랩 과제서 실제 실패·허위완료·하드코딩
   플래그를 내는지 fee-calc 3세션으로 확인(진행 예정). 되면 arm B 성립.
2. **본 수집(W13) 설계 확정** — sonnet: orbit n=40~60(달성·정합) +
   효율 5과제(A/B/parser/fee/roots) 각 ~20~30; haiku: 같은 과제셋에서
   실패 스펙트럼(달성·허위완료·하드코딩). 규모는 power.py 재적용.
3. NDroneFC arm 과제 정의.
**미해결(추적):** sonnet 실패 달성 과제 추후 제작(다중 스케일 절벽형,
orbit 외 도메인 — 유저 명시).
