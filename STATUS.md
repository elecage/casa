...
효율 축 표본으로 확정 전용. 남은 결정(방향 확정 후에만 추가 사용):
- (A) **orbit류 노력형 과제 1~2종 추가** (ODE/시뮬/최적화 — 데모 난이도
  contract + 자가검증 요구). build+각 3세션 보정 사용 소모.
- (B) **모델 계층 arm** (haiku 등 약한 모델에서 달성 실패율↑ → 성공률
  gradient). 유저는 sonnet 확정했으나, "sonnet 천장" 자체가 발견이라
  약모델 교차가 달성 축 데이터를 줄 수 있음.
- (C) **달성/허위완료 축을 orbit 단독으로 수용** + 논문 중심을 효율 축
  (능력 분산, 강한 신호)과 "sonnet 완전명세 천장" 발견으로 재편.
남은 것: NDroneFC arm 과제 정의, 본 수집(W13).
