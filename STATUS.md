...
**W7 (난이도 보정 → 게이트 G2)**. 과제 3종 × 2~3 보정 세션을 러너로
실행해 성공률이 40~80% 구간인지, 행동 지표가 흩어지는지, 위반이
발생하는지 관찰. G1 주의 2건(3/3 성공 = 너무 쉬울 수 있음, 위반 0)이
검증 대상. 모델 선택(본 수집 대상)도 이때 결정해 유저와 상의. 보정
세션은 본 데이터에 불포함. 결과로 G2 기록 + 유저 보고. 필요시 과제
난이도 조정 후 재보정. W6(casa report 집계)는 W7 데이터가 생긴 뒤가
효율적 — W7 결과를 보고 착수.
...
| W7 | 난이도 보정 (과제당 2~3세션, 40~80% 확인) → **게이트 G2** | 진행 중 (A·B·C 보정 완료, D 보정 남음) | 보정 기록 → 이 파일 |
| W7.5 | 과제 D orbit-propagator (숨은 오라클 설계, 유저 제안) | **제작 완료** (2026-07-23, PR #14) — 보정 대기 | `pilot/tasks/orbit-propagator/` |
