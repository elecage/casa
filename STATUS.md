...
**W5 (궤적 지표 확장)**. `src/casa/metrics.py`에 RQ2/RQ3용 궤적 수준
지표 추가: (1) 스텝별 누적 시계열 — k스텝 시점의 탐색 폭·커버리지·에러율·
위반 수 (AUROC@k 곡선의 입력), (2) 세션 간 궤적 유사도 — 도구 호출
시퀀스의 정규화 prefix 편집 거리. PILOT_DESIGN "측정 지표 (궤적 수준)"
절이 사양. audit/CLI 출력에 시계열 포함 여부는 크기 고려해 결정
(별도 --trajectory 플래그 권장). 완료 후 W7(보정, G2).
