...
효율 축 표본으로 확정 전용. 유저 승인(orbit류 과제 추가)에 따라 진자 과제를 설계하려 실측 →
**일반 ODE로는 sonnet 실패 불가**(30주기서 순진 RK4 dE 5e-8 < 심플렉틱
4e-6; RK4가 정확). orbit의 실패는 **다중 스케일 절벽**(고이심률 근점
과소해상) 때문이지 노력형이라서가 아님. 저이심률로 낮추면 RK4가 통과해
분리 소멸 → 중간 난이도 orbit 변형도 불가(난이도 절벽). MAIN_EXPERIMENT
§8. **따라서 신뢰 경로는 둘:**
- (A) **orbit + 세션 증량**(n=40~60; 이미 27%·허위완료 12/13). 신규
  과제 없이 orbit 하나로 달성·정합 축, 나머지는 효율 축.
- (B) **약모델 arm(haiku)**: 모든 과제서 실패 발생 → 달성·허위완료
  풍부 + "sonnet 천장" 대조가 발견. 유저 sonnet 확정과 병행(교차 계층).
→ **유저 재결정 필요**(진자 실측이 앞선 (A)선택의 전제를 뒤집음).
남은 것: 결정 후 NDroneFC arm 정의, 본 수집(W13).
