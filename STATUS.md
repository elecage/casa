...
- **W9 분석 1순위 단서 (대화에서만 관찰된 것):** C(rename-sweep)의 보정
  배치는 세션당 84~160초였는데 본 배치는 620~930초 — 같은 설정에서 몇
  시간 간격의 두 배치가 "빠르고 엉성(1/3 성공)" vs "느리고 철저(13/15
  성공)"로 갈렸다. 소요시간 4~6배 차이는 표본 오차로 설명 불가.
  **분석 시 세션별 model_versions 필드 대조부터** (서빙측 변화 가능성 —
  교란 변수이자 그 자체로 배치 수준 변동이라는 발견 후보). D도 429 중단
  전(1~7)/재개 후(8~15) 소요시간·성공률 비교할 것.
