...
**→ 표적이 바뀌었다: 결과 예측이 아니라 행동 병리 탐지.**
**먼저 읽을 것: `docs/BADNESS_SIGNALS.md`(행동 지표 48종·근거·우선순위),
그다음 `docs/W15_ABILITY_EARLY.md` 1절(아직 미해결인 과제 설계 문제).**

### 다음에 할 일 (2026-08-19 기준)

1. **`진전`의 판정 방식을 정한다.** 호출 하나가 끝났을 때 "무엇이 새로
   생겼나"(새 파일 읽음/파일 내용 변경/에러 감소/테스트 결과 변화)를
   결정론적으로 판정하는 규칙. 이것이 단일 최대 병목 — 헛돎·검증·에러대응
   계열 지표가 전부 여기에 걸려 있다.
2. 우선순위 지표 4종을 `casa.metrics`에 구현하고 **기존 127세션에 돌린다**
   (새 수집 불요, 잠금 유지). 검증 표적: `results/main2/ml-shift-sonnet`의
   퇴화 사례 1건(도구 호출 248회·동일 호출 38회·86분 시간 초과)이 실제로
   잡히는가.
