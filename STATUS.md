...
3. **경보 규칙을 정한다 (미정 — 설계 필요).** 신호가 여럿인데 **언제
   경보를 울릴지가 아직 결정되지 않았다.** 단일 신호인가 조합인가, 조합이면
   어떻게 합치는가, 알림 문턱과 종료 권고 문턱을 각각 어디에 두는가
   (`docs/ARCHITECTURE.md`: 외부 앱이라 문턱이 둘이다). **4번은 이것이
   정해져야 시작된다.**
