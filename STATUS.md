...
**본 실험 재설계 방향 유저 결정 대기.** W9 분석 + 재검토 완료
(`docs/PILOT_RESULTS.md` §1~6 원분석, §7 재검토, §8 능력 분산). 경위:
사전 등록 기준 대부분 미충족 → 유저 지적 2건(시간 지표 부적합, 지표
타당성 미검증)으로 재검토 → 배치 국면 전환은 시사적 관찰로 강등, 검증
행동 지표도 무신호 확인 → 유저가 연구 관심을 명시(세션 능력 차이의 조기
판별로 토큰 절약, PILOT_RESULTS §8). 유력 방향: **결과변수를 성공 여부
에서 "완료까지의 토큰·턴(능력)"으로 재정의** — 같은 과제 성공 세션끼리도
비용 2~4배 차이(§8a), 포화 과제(A·B)도 유효 표본화, B에서 조기 신호 확인
(ρ≈−0.6). 보조 후보: 세션 간 diff 합의도, 위반 축, in-the-wild(유저 실
프로젝트 세션 관찰). 유저 승인 시: RESEARCH_PLAN/PILOT_DESIGN 개정(W10)
부터 착수.
