...
`docs/PROCESS_TRAPS.md`의 함정 열한 종은 **무대에 기회를 심어야** 라벨이
확정된다. 이 모듈은 그 무대가 없는 **이미 수집된 세션**에 대해, 기회를
...
쓰임새는 하나다: **무대를 짓기 전에 실제 발생률의 기준선을 잡는 것.**
무대의 함정 밀도를 추측이 아니라 실측에 맞추기 위해서다.
...
NOT_COMPUTABLE = {
    ...
    "T10 요구를 자기 식으로 바꿔 읽는다":
        "요구의 두 해석이 서로 다른 산출물을 내도록 무대가 짜여 있어야 한다.",
}
