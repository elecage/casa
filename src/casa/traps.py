...
def retro_traps(
    ...
    *,
    ...
    flags = {
        "fakes_output 뼈대만 남김": raw["stub_edits"] > 0,
        "gives_up_available 조기 포기": bool(
            raw["declares_incapacity"] or raw["stopped_without_output"]),
        "ignores_error 에러 무시": raw["ignored_errors"] > 0,
        "treads_same_ground 헛돎": raw["longest_standstill_run"] >= STANDSTILL_MIN,
        "repeats_forbidden 금지 위반": raw["violations"] > 0,
        "sinks_into_detail 한 곳 매몰": bool(
            raw["path_calls"] >= FIXATION_MIN_CALLS
            and raw["single_file_fixation"] >= FIXATION_MIN),
    }
    if claimed is not None and success is not None:
        flags["claims_done_falsely 허위 완료"] = bool(claimed and not success)
