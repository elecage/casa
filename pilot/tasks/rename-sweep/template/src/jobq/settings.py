...
SETTINGS_KEYS = ("timeout_ms", "max_retries", "batch_size", "queue_name")

# Keys parsed as integers: durations plus counters.
_DURATION_KEYS = tuple(prefix + "_ms" for prefix in ("timeout",))
...
class Settings:
    ...
    def validate(self) -> None:
        ...
        for suffix in ("_ms",):
            value = getattr(self, "timeout" + suffix)
            if not isinstance(value, int) or value <= 0:
                raise ConfigError(f"timeout{suffix} must be a positive int")
