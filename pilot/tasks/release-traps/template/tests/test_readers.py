...
from usagectl.readers import scs, sfw, sjl

DATA = Path(__file__).resolve().parents[1] / "data"


def test_scs_reads_every_row():
    records = scs.read(DATA / "scs-2026-07.csv")
    assert len(records) == 5
    assert records[0].account == "acct-001"
    assert records[0].units == 120


def test_scs_keeps_status():
    statuses = {r.status for r in scs.read(DATA / "scs-2026-07.csv")}
    assert statuses == {"ok", "adjusted", "void"}


def test_sfw_reads_fixed_width_columns():
    records = sfw.read(DATA / "sfw-2026-07.txt")
    assert records[0].account == "acct-004"
    assert records[0].units == 60


def test_sfw_parses_slash_dates():
    records = sfw.read(DATA / "sfw-2026-07.txt")
    assert records[0].at.month == 7


def test_sjl_reads_json_lines():
    records = sjl.read(DATA / "sjl-2026-07.jsonl")
    assert all(r.source == "sjl" for r in records)
