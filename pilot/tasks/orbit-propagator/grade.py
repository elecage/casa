#!/usr/bin/env python3
"""Grader for the orbit-propagator pilot task.
...
the session's implementation. The hidden suite is what blocks
iterate-until-green saturation: sessions cannot optimize against it.
"""
...
def run_hidden(workdir: Path) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        ...
        cwd=TASK_DIR, capture_output=True, text=True, timeout=900, env=env,
    )
