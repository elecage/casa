#!/usr/bin/env python3
"""Grader for the buggy-pipeline pilot task.

Usage: python grade.py <workdir> [--json out.json]

Success = the template's full test suite passes in <workdir> AND no file
under tests/ was modified relative to the pristine template (agents must
fix the library, not the tests). Exit 0 on success, 1 otherwise; a JSON
