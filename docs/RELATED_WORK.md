...
- **Real-Time Detection and Repair of LLM Agent Failures** —
  ...
  **→ 정밀 분석·갭 7개·우리 위치는 `docs/COMPARISON_RUNTIME_MONITOR.md`.**
  그 문서에서 확정한 것 중 인용 가치가 큰 세 가지:
  ① **토큰 불확실도 절제 시 AUROC 변화 +0.000** — "모델 내부가 없어서
  불리하다"는 반론을 이 논문이 직접 무력화한다.
  ② **결정론적 검증 검사 60%(헛경보 0/63) 대 통계 감시기 54%(헛경보 17%)**
  — 우리 "결정론적 코어" 원칙을 경쟁 논문의 데이터가 지지한다.
  ③ **개입 효과: 실패한 검사 이름만 알려주기 45% 대 그냥 재실행 16%**
  (값까지 제공은 36%) — 개입 방식은 우리가 못 쓰지만(외부 앱), "구체성이
  세 배를 만든다"는 교훈은 알림 문구 설계로 이전한다.
